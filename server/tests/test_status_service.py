...
class TestStatusServiceSituations(unittest.TestCase):
    ...
    def test_situation_is_not_expired_and_has_started(self):
        start_time = datetime.now(pytz.timezone('US/Eastern')) - timedelta(days=2)
        end_time = datetime.now(pytz.timezone('US/Eastern')) + timedelta(days=1)
        xml = self.__get_xml_with_args('situation_no_summary.xml', format_time(start_time), format_time(end_time))
    ...
    def test_situation_with_resumed_summary(self):
        start_time = datetime.now(pytz.timezone('US/Eastern')) - timedelta(days=2)
        end_time = datetime.now(pytz.timezone('US/Eastern')) + timedelta(days=1)
        xml = self.__get_xml_with_args('situation_with_summary.xml', format_time(start_time), format_time(end_time), "Service has Resumed")
        requests.get = MagicMock(return_value=MockedResponse(str.encode(xml)))
        self.status_service.update_status()
        situations = self.status_service.situations
        self.assertFalse(situations) # The start date is in the future
    ...
    def __get_xml_with_args(self, file_name, *args):
        with open(os.path.abspath(os.path.join('tests', 'data', file_name)), 'r') as xml:
            raw_xml = xml.read()
